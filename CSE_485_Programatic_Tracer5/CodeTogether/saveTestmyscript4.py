...
class Python_tacer():
    ...
    def setFilePath(self, filepath_set):
        ...
        self.lastlineofprogram = self.getlastline(self.filepath_var[1:])
...
def breakpoint(linenumberlist):
    #print("num of threads = ", threading.active_count())
    #lastline = ''
    #lastline = linecache.getline(thing1.filepath_var[1:], thing1.cur_frame.f_lineno)
    # linenum = 1
    line = "a"
    if thing1.cur_frame != "":
        while thing1.cur_frame.f_lineno not in linenumberlist and thing1.cur_frame.f_lineno != thing1.lastlineofprogram:
            #print("thing1.lastlineofprogram = ",thing1.lastlineofprogram)
            step()
            line = linecache.getline(thing1.filepath_var[1:], thing1.cur_frame.f_lineno-1)
            #print("line = ", line)
            #linenum = thing1.cur_frame.f_lineno-1
            #print("lineno = ", thing1.cur_frame.f_lineno-1)
    else:
        step()
        breakpoint(linenumberlist)
...
def stepover():
    ...
    if thing1.cur_event != -1:        
        ...
        #step()
        if thing1.cur_frame.f_lineno == 1:
            step()
            #step()
        else:
            step()
            ...
            step()
    #else:
    #   print("in stepover(), taking a single step()")
    #   step()
    #   stepover()
